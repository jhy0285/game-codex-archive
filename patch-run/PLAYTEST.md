...
- **Exercised:** opened the canonical Drowned Scriptorium build, issued movement, Threadstep, pointer aim/fire input, observed multiple Seed Husks and the full-screen death/restart prompt, and checked browser error output.
- **Visual result:** the player, pursuing husks, top rule/HUD rows, route selector, and death action remained legible at 1280x720. No browser console error was observed.
- **Automation paired with this pass:** Vitest 5/5, production build, and Playwright 3/3 all passed again on the new PC; automation covers all three 20/40/60-second rules and clean restart.
- **Human-only boundary:** physical control feel, cyan EVADE versus red damage clarity under pressure, motion responsiveness, target-machine performance, and an uninterrupted real-time 65-second comprehension run still require a person.
