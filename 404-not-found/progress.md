...
## 2026-08-14 -- new-PC reproduction and mobile HUD QA

- Installed from `package-lock.json` with `npm ci` and installed Playwright Chromium 1.62.1.
- Reproduced the archived baseline: Vite production build PASS and Playwright 7/7 PASS, including all five local three-act scenarios and both production scenarios.
- Interactive in-app-browser agent QA exercised the start flow, desktop movement/jump/dash/strike inputs, and the 390x844 touch layout. This was agent-observed QA, not a human playtest; no console error was observed.
- Found one presentation-only Gate 1 defect at 390x844: the 2.2-second start toast overlapped the right-side Act objective. Moved the mobile toast below the objective and added a bounding-box non-overlap assertion to the existing first Playwright scenario. Mechanics, world coordinates, timings, controls, touch-target sizes, and production routing remain unchanged.
- Post-fix verification: `npm run build` PASS and the complete Playwright suite PASS 7/7. The corrected mobile frame was inspected directly. No deployment was attempted.
- Still requires a human: physical-keyboard jump/dash/strike feel and real-finger touch spacing/comfort on a phone.
