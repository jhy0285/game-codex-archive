...
The Watcher owns a real world position and facing. Each fixed tick it tests both Player and Echo against range, state-dependent FOV, and a cover ray; chooses an actually visible target; preserves last-known/stimulus positions for investigation; and returns through recovery to its patrol. `lure-bell` supplies only a world-space sound stimulus—the bell does not assign a target or defeat state. Patrol, chase, investigation, and knockback displacement use three parallel wall/door clearance rays across the enemy's width. When the desired chase/investigation vector is blocked, deterministic side probes choose the clearest useful arc around the corner; no random path choice or snapshot state is added. Physical knockback remains a straight clearance-limited impulse and cannot steer around cover into the trap. The upper flank is reached by one long, gentle, walkable ramp, and the Watcher has no conventional damage goal. A continuous inset foundation now spans the gallery inside its perimeter walls, so route-reading mistakes do not cause unrelated falls; the raised safe, Echo, patrol, ramp, and exit slabs remain visually distinct and retain their authored cover and height relationships.
...
6. Reach `upper-seal` and hold `E` while the Echo still physically occupies `lower-seal`. The live simultaneous occupancy latches final-door release; the door slides east out of the passage, then the full 15-second escape begins once its collider is actually clear.
