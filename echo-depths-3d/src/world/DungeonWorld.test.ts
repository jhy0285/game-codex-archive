...
describe('DungeonWorld authored runtime contracts', () => {
  ...
  it('lets the shared capsule motor descend, cross the powered Chapter 3 bridge, and reach the east side', async () => {
    const { physics, world } = await createWorld(3)
    const record = physics.createActor('atrium-player', 'player', { x: -6.1, y: 3.58, z: 3.3 })
    const motor = new CharacterMotor(physics, record)
    try {
      const player = actor('atrium-player', 'player', [-6.1, 3.58, 3.3])
      const echo = actor('echo', 'echo', [-5.4, 3.2, -1.1])
      echo.interactHeld = true
      expect(world.interact(echo)).toBe('lever')

      const advance = (input: { moveX: number; moveZ: number }, ticks: number, startTick: number) => {
        for (let tick = startTick; tick < startTick + ticks; tick += 1) {
          world.beforePhysics(tick, [player, echo])
          motor.prepare({ ...input, jumpPressed: false, dashPressed: false })
          physics.step()
          motor.syncAfterStep()
          player.position.copy(motor.position)
          world.afterPhysics([player, echo])
      ...
      advance({ moveX: 0, moveZ: -1 }, 42, 1)
      advance({ moveX: 1, moveZ: 0 }, 72, 43)
      advance({ moveX: 0, moveZ: -1 }, 20, 115)
      advance({ moveX: 1, moveZ: 0 }, 92, 135)

      const bridgeBody = physics.record('rotating-bridge')?.body
      const traversal = {
        x: motor.position.x,
        y: motor.position.y,
        z: motor.position.z,
        grounded: motor.grounded,
        bridgePosition: bridgeBody?.translation(),
        bridgeRotation: bridgeBody?.rotation(),
      }
      expect(world.debugState().facts).toContain('bridge-lever-echo')
      expect(motor.position.x, JSON.stringify(traversal)).toBeGreaterThan(5.2)
      expect(motor.position.y).toBeGreaterThan(0.9)
      expect(motor.grounded).toBe(true)
    } finally {
      motor.dispose()
      world.dispose()
  ...
  it('clears the full Chapter 3 core route through a real echo throw, catch, redirect, receiver, and exit request', async () => {
    const { physics, world } = await createWorld(3)
    try {
      const echo = actor('echo', 'echo', [-3.8, 3.75, 2.5])
      const player = actor('player', 'player', [0.1, 1.05, 1.15])
      ...
      expect(world.throwOrDrop(echo, new THREE.Vector3(1, 0, -0.34))).toBe('core')
      echo.position.set(-5.4, 3.2, -1.1)
      echo.interactHeld = true
      expect(world.interact(echo)).toBe('lever')

      for (let tick = 2; tick < 116 && !world.isCarrying('player', 'core'); tick += 1) {
        stepWorld(world, physics, tick, [player, echo])
      }
      expect(world.isCarrying('player', 'core'), JSON.stringify(world.debugState())).toBe(true)
      expect(world.debugState().facts).toEqual(expect.arrayContaining(['bridge-lever-echo', 'core-thrown-by-echo', 'core-caught']))

      player.position.set(5.45, 1.05, 1.6)
      player.facingYaw = Math.PI / 2
      player.carryYaw = Math.PI / 2
      stepWorld(world, physics, 116, [player, echo])
      expect(world.throwOrDrop(player, new THREE.Vector3(1, 0, 0))).toBe('core')
      expect(world.attack(player, new THREE.Vector3(1, 0, 0))).toBe('core')
      for (let tick = 117; tick < 180 && !world.debugState().cores['memory-core']?.receiver; tick += 1) {
        stepWorld(world, physics, tick, [player, echo])
      }
      stepWorld(world, physics, 180, [player, echo])

      const delivered = world.debugState()
      expect(delivered.facts).toEqual(expect.arrayContaining(['core-redirected', 'receiver-filled', 'core-route-complete']))
      expect(delivered.doors['atrium-door']?.open).toBe(true)
      player.position.set(10.2, 1.08, -0.4)
  ...
  it('requires a catch followed by a live redirect before the Chapter 3 receiver', async () => {
    const { physics, world } = await createWorld(3)
    try {
      const core = physics.record('memory-core')
      const receiver = CHAPTER_LAYOUTS[3].devices.find((device) => device.id === 'core-receiver')?.position
      if (!core || !receiver) throw new Error('Chapter 3 core or receiver is missing')

      core.body.setTranslation({ x: receiver[0], y: receiver[1], z: receiver[2] }, true)
      core.body.setLinvel({ x: 0, y: -1, z: 0 }, true)
      physics.step()
      world.afterPhysics([])
      expect(world.debugState().cores['memory-core']?.receiver).toBe(false)
      expect(world.debugState().facts).not.toContain('receiver-filled')

      core.body.setTranslation({ x: 0, y: 1.1, z: 0 }, true)
      core.body.setLinvel({ x: 1, y: 0, z: 0 }, true)
      physics.step()
      world.afterPhysics([])
      expect(world.attack(actor('player', 'player', [0, 1.1, 0]), new THREE.Vector3(1, 0, 0))).toBeUndefined()
      expect(world.debugState().facts).not.toContain('core-redirected')

      const catcher = actor('player', 'player', [0, 1.1, 0])
      world.facts.add('core-thrown-by-echo')
      expect(world.interact(catcher)).toBe('core')
      expect(world.throwOrDrop(catcher, new THREE.Vector3(1, 0, 0))).toBe('core')
      expect(world.attack(actor('player', 'player', [0, 1.1, 0]), new THREE.Vector3(1, 0, 0))).toBe('core')
      core.body.setTranslation({ x: receiver[0], y: receiver[1], z: receiver[2] }, true)
      core.body.setLinvel({ x: 0, y: -1, z: 0 }, true)
      physics.step()
      world.afterPhysics([])

      expect(world.debugState().facts).toEqual(expect.arrayContaining(['core-redirected', 'receiver-filled', 'core-route-complete']))
      expect(world.debugState().cores['memory-core']?.receiver).toBe(true)
      world.facts.delete('core-redirected')
      world.facts.delete('core-route-complete')
      expect(world.attack(actor('player', 'player', receiver), new THREE.Vector3(1, 0, 0))).toBeUndefined()
      expect(world.debugState().facts).not.toContain('core-redirected')
      expect(world.debugState().objectiveFacts).toContain('core-route-complete')
    } finally {
      world.dispose()
      physics.dispose()
    }
