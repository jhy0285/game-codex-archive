...
- Source `5d04c0897c810a261641d37a4f77181cda1364dd` is on `origin/main`. Candidate `dpl_Bkdp37fWGdxnRV2LMu5FECTBcVzd` passed the complete hosted smoke on rerun after one pre-existing Chapter 1 path flake, then promoted production `dpl_BisUo1jHoNnXg73CANdkinMg96QN` became Ready. The public [production URL](https://echo-depths-3d.vercel.app) serves `index-DT-5l4KY.js`, passes smoke 6/6, enters Chapters 3–5 with all five cards active, renders Chapter 5 mobile controls, exposes no debug API, and collected no browser errors.
